 ...
 *
 * A token is `latha_` + 256 random bits (base64url). Only the SHA-256 hex of
...
export const API_KEY_TOKEN_PREFIX = 'latha_'

/** `latha_` + 8 token chars — enough to identify a key in a list. */
const DISPLAY_PREFIX_LENGTH = API_KEY_TOKEN_PREFIX.length + 8

const textEncoder = new TextEncoder()

/** Generate a fresh API key token: `latha_<base64url(32 random bytes)>`. */
...
/** The identifying prefix persisted alongside the hash (e.g. `latha_Ab12Cd34`). */
