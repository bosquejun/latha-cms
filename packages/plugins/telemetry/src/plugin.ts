  ...
  return {
    ...
    onInit(cms: Kon10Instance) {
      ...
      if (firstRun || !store.notified) {
        cms.logger.info(
          { plugin: 'telemetry' },
          'Kon10 collects anonymous usage telemetry to help improve the product. ' +
            'No content, credentials, or personal data are collected. ' +
            'Opt out any time with KON10_DISABLE_TELEMETRY=1 (or DO_NOT_TRACK=1).',
        )
        markNotified(env)
      }
