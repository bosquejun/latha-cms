...
|---|---|---|
| `@kon10/core` | Kernel primitives: registry, hook engine, access evaluator, module lifecycle, field type registry contract | Module-specific logic, admin/UI concerns, field types owned by modules |
...
| `@kon10/admin-sdk` | Admin UI shell, field renderers, views, display hints | Business logic, persistence logic |
...
### Adding admin UI
Follow the admin extension convention (`src/admin/widgets/`, `src/admin/pages/`, etc.) or use `defineAdminExtensions` explicitly. No module should write directly to `@kon10/admin-sdk` internals.
